...
//            Log.info(vNew.domain()[i] + " -> " + meanWeightedResponse[idx[i]]);
  ...
  private Frame adaptFrameToTrain(Frame fr, String frDesc, String field, boolean expensive, boolean catEncoded) {
    ...
    try {
      String[] msgs = Model.adaptTestForTrain(adapted, catEncoded ? _train._names : null, null, _train._names, _train.domains(), _parms, expensive, true, null, getToEigenVec(), _workspace.getToDelete(expensive), false);
